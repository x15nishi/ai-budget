...
DATA_FILE = "data.csv"
EXPENSE_COLS = ["Date", "Category", "Remark", "Amount"]
...
#=========================STEP 4: LOAD DATA + SESSION STATE========================#
# expenses are saved to data.csv so they are not lost on refresh
def load_expenses():
    """This function loads saved expenses from
    data.csv, and makes sure Amount is numeric"""
    try:
        df = pd.read_csv(DATA_FILE)
        if list(df.columns) != EXPENSE_COLS:
...
def save_expenses(df):
    """This function saves the current expense
    table back to data.csv"""
...
#=====================STEP 6: TABS FOR REST OF THE APP====================
...
with tab_add:
    ...
    with col3:
        remark = st.text_input("Remark (optional)", placeholder = "e.g. Zomato order")
    with col4:
    ...
    if add_clicked:
        ...
        if amount > 0:
            new_row = pd.DataFrame([{"Date": expense_date.isoformat(), "Category": final_category, "Remark": remark, "Amount": amount}])
    ...
    if st.session_state.expenses.empty:
        ...
        if st.button("🗑️ Clear All"):
            st.session_state.expenses = pd.DataFrame(columns = EXPENSE_COLS)
            save_expenses(st.session_state.expenses)
            st.rerun()

    st.divider()
    st.subheader("Category Budget Limits (optional)")
    st.caption("Set a monthly limit per category to get a progress warning. This is not saved after you close the app.")

    limit_cols = st.columns(4)
    categories_used = ["Food", "Rent", "Transport", "Shopping"]
    for i, cat in enumerate(categories_used):
        with limit_cols[i % 4]:
            st.session_state.budgets[cat] = st.number_input(f"{cat} Limit (₹)", min_value = 0.0, step = 500.0, key = f"limit_{cat}")
...
with tab_dash:
    ...
    if not st.session_state.expenses.empty:
        ...
        with chart_col2:
            ...
            ax_bar.bar(category_totals.index, category_totals.values, color = "#3D7EDB")
...
with tab_forecast:
    ...
    st.caption("Uses your real monthly history when available, otherwise a simple projection from current numbers.")

    if not st.session_state.expenses.empty and monthly_income > 0:
        ...
        if len(monthly_totals) > 1:
            st.write("**Actual Monthly Expense History**")
            fig_hist, ax_hist = plt.subplots()
            ax_hist.plot(monthly_totals.index, monthly_totals.values, marker = "o", color = "#D9534F")
            ax_hist.set_ylabel("Expense (₹)")
            plt.xticks(rotation = 30)
            st.pyplot(fig_hist)
            avg_monthly_expense = monthly_totals.mean()
        else:
            st.info("Add expenses across more than one date to see real history. Showing a simple projection for now.")
            avg_monthly_expense = total_expense

        forecast_type = st.radio("Forecast Horizon", ["Monthly", "Yearly"], horizontal = True)
        projected_savings_per_period = monthly_income - avg_monthly_expense

        if forecast_type == "Monthly":
            periods = list(range(1, 7))
            projected = [projected_savings_per_period * p for p in periods]
            x_label = "Next 6 Months"
        else:
            periods = list(range(1, 6))
            projected = [projected_savings_per_period * 12 * p for p in periods]
            x_label = "Next 5 Years"

        fig2, ax2 = plt.subplots()
        ax2.plot(periods, projected, marker = "o", color = "#1E7F4F")
        ax2.set_xlabel(x_label)
        ax2.set_ylabel("Projected Savings (₹)")
        ax2.set_title("Savings Trend Projection")
        st.pyplot(fig2)

        st.write(f"Average Monthly Expense Used : ₹{avg_monthly_expense:,.2f}")
        st.write(f"Projected Yearly Expense : ₹{avg_monthly_expense * 12:,.2f}")
        st.write(f"Projected Yearly Savings : ₹{projected_savings_per_period * 12:,.2f}")
    else:
        st.info("Enter income and add expenses to see forecast")
...
with tab_chat:
    ...
    def get_budget_context_chat():
        """Same context builder, used inside the chat tab"""
        expense_summary = st.session_state.expenses.groupby("Category")["Amount"].sum().to_string() if not st.session_state.expenses.empty else "No expenses recorded"
        return f"""
        Monthly Income: ₹{monthly_income}
        Total Expense: ₹{total_expense}
